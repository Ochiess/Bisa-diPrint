...
if( isset($_COOKIE['key']) && isset($_COOKIE['oci']) ) {
   ...
   $result = mysqli_query($conn, "SELECT email FROM user WHERE id = $key");
    ...
    if(mysqli_num_rows($result)===1 ) {
        ...
        if(password_verify($password, $row["password"]) ) {
            ...
            if( isset($_POST['remember']) ) {
                ...
                setcookie('photo', hash('sha256',$row['photo']), strtotime('+7 days'),'/');
                setcookie('nama_akun', hash('sha256',$row['nama_akun']), strtotime('+7 days'),'/');
                setcookie('hp', hash('sha256',$row['hp']), strtotime('+7 days'),'/');
            }
            header("Location: index.php");
            exit;
        }
...
<head>
    ...
    <title>diPrint - Login User</title>
                                ...
                                <div class="p-5">
                                    ...
                                    <form action="" class="user" id="form-login" method="POST">
                                        <div class="form-group">
                                            <input for="email" type="email" name="email" id="email" class="form-control form-control-user" id="email" aria-describedby="emailHelp" placeholder="Alamat Email...">
                                        </div>
                                        <div class="form-group">
